...
`replicator:hook(entity, action, ...)`. The arguments of this method will change depending on the action.
...
but calling `replicator:override(entity, action, ...)` instead, you cant override and hook at the same time for the same replication pair and action.
