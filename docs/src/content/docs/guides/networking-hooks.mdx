...
You can set hooks to when an action is performed. To do this, you need to call 
`replicator:hook(action, ...)`. The arguments of this method will change depending on the action.

This method returns a disconnect function that you can call to remove the hook.

<div class="padded">
### Component Changes
Setting the action to "changed" will make the callback run right after a component has been changed.
...
- replication_pair: `Pair` <span class="sub-heading" style="white-space: pre;"> - what component to hook to. This pair should mimic how you track components [in the server](/replecs/guides/networking-entity/#replication-types)</span> 
...
replicator:hook("changed", jecs.pair(replecs.reliable, components.speed), function(entity, id, speed)
    print(entity, "changed speed to", speed)
end)
replicator:hook("changed", jecs.pair(replecs.pair, components.attacking), function(entity, id)
    print(entity, "is now attacking", jecs.pair_second(world, id)) 
end)
```

### Component Removals
Setting the action to "removed" will make the callback run right before a component or pair gets removed.
...
replicator:hook("removed", jecs.pair(replecs.reliable, components.speed), function(entity, id)
    print("removed speed for:", entity, world:get(entity, components.speed))
end)
replicator:hook("removed", jecs.pair(replecs.pair, components.attacking), function(entity, id)
    print(entity, "is no longer attacking", jecs.pair_second(world, id)) 
end)
```


### Entity Deletions
Setting the action to "deleted" will make the callback run right before an entity gets deleted. You need to connect this once per-entity.
All hooks get disconnected automatically after the entity is deleted.
...
replicator:hook("deleted", entity, function(entity)
...
You can override actions. When this happens, replecs will not perform the action, but instead run the callback.
This allows you add your own logic for changing/removing/deleting entities and components. 

You override actions the same way you would hook to them,
but calling `replicator:override(entity, action, ...)` instead, you cant override and hook at the same time for the same component and action.

All actions behave the same way with the exception of "changed" since the component will never change.

```luau
local replicator = require("@client/replicator")

replicator:override("changed", jecs.pair(replecs.reliable, components.speed), function(entity, _, speed)
    -- double the speed in the client (for some reason?)
    world:set(entity, components.speed, speed * 2)
end)
replicator:override("removed", jecs.pair(replecs.reliable, components.speed), function(entity)
    -- set speed to 0 instead of removing it
    world:set(car, components.speed, 0)
end)
replicator:override("deleted", enemy, function(_)
    -- fade out the enemy and add lifetime to be deleted after 4 seconds
    world:add(enemy, components.fade_out)
    world:set(enemy, components.lifetime, 4)
end)
```
